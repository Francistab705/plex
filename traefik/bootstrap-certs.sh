...
# Put certs inside the revan folder so all paths line up with compose
CERTS_DIR="${CERTS_DIR:-./revan/certs}"
mkdir -p "$CERTS_DIR" revan/traefik revan/env.d

# Default domains: wildcard for nip.io plus explicit service hostnames
DOMAINS_DEFAULT="localhost 127.0.0.1 ::1 *.127.0.0.1.nip.io traefik.127.0.0.1.nip.io archon.127.0.0.1.nip.io archon-api.127.0.0.1.nip.io supabase.127.0.0.1.nip.io studio.127.0.0.1.nip.io llm.127.0.0.1.nip.io"
...
cat > revan/traefik/certs.yaml <<'YAML'
tls:
  certificates:
...
CA_ENV_FILE="revan/env.d/ca.env"
cat > "$CA_ENV_FILE" <<'ENV'
...
echo "✅ Traefik TLS file written to revan/traefik/certs.yaml"
...
#     - ./revan/env.d/ca.env
#   volumes:
#     - ./revan/certs/rootCA.pem:/usr/local/share/ca-certificates/mkcert-root.crt:ro
#
